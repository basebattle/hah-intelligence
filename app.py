...
# CSS injection — st.markdown is the correct mechanism for global styles
st.markdown("""
<style>
...
.hero-badge {
    ...
    padding:0.35rem 1.1rem; font-family:'JetBrains Mono',monospace;
    font-size:0.72rem; color:var(--teal); letter-spacing:0.12em;
...
.hero-subtitle {
    ...
    margin:0 auto 0.75rem; line-height:1.7; font-weight:300;
}
.hero-attribution { font-size:0.88rem; color:var(--muted); margin-bottom:2rem; }
.hero-attribution a {
    color:var(--teal); text-decoration:none;
    border-bottom:1px solid rgba(0,191,165,0.35); padding-bottom:1px;
}
.hero-pills { display:flex; justify-content:center; gap:0.6rem; flex-wrap:wrap; margin-top:1.5rem; }
.pill {
    background:var(--surface); border:1px solid var(--border); border-radius:999px;
    padding:0.38rem 0.95rem; font-size:0.78rem; color:var(--muted);
    font-family:'JetBrains Mono',monospace; letter-spacing:0.03em;
}
.pill-teal { border-color:var(--teal-border); color:var(--teal); background:var(--teal-dim); }
.stats-bar {
    display:flex; justify-content:center; margin:2.5rem auto; max-width:760px;
    border:1px solid var(--border); border-radius:16px; overflow:hidden; background:var(--surface);
}
.stat-item { flex:1; text-align:center; padding:1.4rem 0.5rem; border-right:1px solid var(--border); }
.stat-item:last-child { border-right:none; }
.stat-num { font-family:'DM Serif Display',serif; font-size:1.9rem; color:var(--teal); line-height:1; }
.stat-label { font-size:0.68rem; color:var(--muted); text-transform:uppercase; letter-spacing:0.1em; margin-top:0.4rem; }
.section-header { font-family:'DM Serif Display',serif; font-size:1.65rem; color:var(--text); margin:2.5rem 0 0.4rem; line-height:1.2; }
.section-sub { font-size:0.88rem; color:var(--muted); margin-bottom:1.5rem; }
.condition-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(185px,1fr)); gap:1rem; margin:1.25rem 0 2rem; }
.condition-card {
    background:var(--surface); border:1px solid var(--border); border-radius:16px;
    padding:1.6rem 1.25rem; text-align:center;
    transition:border-color 0.3s ease,box-shadow 0.3s ease,transform 0.2s ease;
}
.condition-card:hover { border-color:var(--teal-border); box-shadow:var(--teal-glow); transform:translateY(-3px); }
.condition-icon-wrap { font-size:2.6rem; display:block; margin-bottom:0.85rem; line-height:1; }
.condition-name { font-weight:600; font-size:0.92rem; color:var(--text); margin-bottom:0.3rem; }
.condition-desc { font-size:0.76rem; color:var(--muted); line-height:1.5; }
.module-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(230px,1fr)); gap:1rem; margin:1.25rem 0 2rem; }
.module-card {
    background:var(--surface); border:1px solid var(--border);
    border-left:3px solid var(--teal); border-radius:12px; padding:1.5rem;
    transition:box-shadow 0.3s ease,transform 0.2s ease;
}
.module-card:hover { box-shadow:var(--teal-glow); transform:translateY(-2px); }
...
.hiw-wrap { background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:1.75rem; margin-bottom:1.5rem; }
.how-step { display:flex; align-items:flex-start; gap:1.1rem; padding:1rem 0; border-bottom:1px solid var(--border); }
.how-step:last-child  { border-bottom:none; padding-bottom:0; }
.how-step:first-child { padding-top:0; }
.step-num {
    min-width:2rem; width:2rem; height:2rem; background:var(--teal-dim);
    border:1px solid var(--teal-border); border-radius:50%;
...
.portfolio-card { background:var(--surface); border:1px solid var(--teal-border); border-radius:16px; padding:1.75rem; text-align:center; }
.btn-teal {
    display:inline-block; background:var(--teal); color:#000 !important;
    padding:0.55rem 1.4rem; border-radius:8px; font-size:0.82rem;
    font-weight:600; text-decoration:none !important; letter-spacing:0.02em;
}
.footer { text-align:center; padding:2.5rem 1rem; border-top:1px solid var(--border); margin-top:4rem; color:var(--muted); font-size:0.82rem; }
.footer a { color:var(--teal); text-decoration:none; }
.footer-quote { font-family:'DM Serif Display',serif; font-style:italic; font-size:0.92rem; color:rgba(255,255,255,0.28); margin-top:1rem; line-height:1.5; }
...
@keyframes temp-throb  { 0%,100%{filter:hue-rotate(0deg)} 50%{filter:hue-rotate(30deg) brightness(1.2)} }
...
.stButton > button {
    background:var(--teal-dim) !important; border:1px solid var(--teal-border) !important;
    color:var(--teal) !important; border-radius:8px !important; font-family:'DM Sans',sans-serif !important;
}
...
# ── HERO ──────────────────────────────────────────────────────────────────────
# st.html() bypasses the CommonMark parser — prevents blank-line HTML-block truncation
st.html("""
<div class="hero-wrap">
...
st.html('<div style="border-top:1px solid rgba(255,255,255,0.07);margin:0.5rem 0 2rem;"></div>')
...
<div class="module-grid">
    <div class="module-card">
        ...
        <div class="module-desc">Live 20-patient cohort with condition icons, NEWS2 scores, and one-click telemetry refresh. Red / Amber / Green risk stratification at a glance.</div>
    </div>
    <div class="module-card">
        <div class="module-icon">🩻</div>
        <div class="module-title">Patient Detail</div>
        <div class="module-desc">Drill into any patient — full vitals panel, AI confidence gauge, LLM-synthesised risk reasoning, and clinical notes.</div>
    </div>
    <div class="module-card">
        <div class="module-icon">🚨</div>
        <div class="module-title">Active Alerts</div>
        <div class="module-desc">Auto-generated escalation queue ranked CRITICAL → HIGH → MEDIUM. Each alert carries a one-line action summary and patient context.</div>
    </div>
    <div class="module-card">
        <div class="module-icon">📋</div>
        <div class="module-title">CMS Compliance</div>
        <div class="module-desc">Real-time AHCaH waiver protocol tracking. Met / At-Risk / Failed per requirement, with audit notes and remediation guidance.</div>
    </div>
</div>
""")

st.html('<div style="border-top:1px solid rgba(255,255,255,0.07);margin:0.5rem 0 2.5rem;"></div>')
...
<div class="portfolio-card">
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.72rem;color:#00BFA5;letter-spacing:0.1em;margin-bottom:1rem;">P10 OF 10</div>
    <div style="font-family:'DM Serif Display',serif;font-size:1.2rem;color:#fff;margin-bottom:0.25rem;">Dr. Piyush Sharma</div>
    <div style="font-family:'DM Serif Display',serif;font-style:italic;font-size:0.95rem;color:rgba(255,255,255,0.52);margin-bottom:0.85rem;">PT, MHA</div>
    ...
    <p style="font-size:0.8rem;color:rgba(255,255,255,0.52);line-height:1.55;margin-bottom:1.25rem;">
        Senior Healthcare Consultant at the intersection of clinical AI and US payer-provider operations. 10 production systems. $2.8M projected annual impact.
    </p>
    <div style="border-top:1px solid rgba(255,255,255,0.07);padding-top:1rem;margin-bottom:1.25rem;text-align:left;">
        <div style="font-size:0.75rem;color:rgba(255,255,255,0.52);line-height:2.0;">
    ...
    <a class="btn-teal" href="https://hc-portfolio-zeta.vercel.app/" target="_blank">View Full Portfolio →</a>
</div>
<div style="background:#0F1923;border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:1.25rem;margin-top:1rem;">
    <div style="font-size:0.7rem;color:#00BFA5;font-family:'JetBrains Mono',monospace;letter-spacing:0.08em;margin-bottom:0.75rem;">TECH STACK</div>
...
st.html("""
<div class="footer">
    <strong style="color:#fff;">HaH Intelligence Layer</strong>
    &nbsp;·&nbsp; P10 of 10
    &nbsp;·&nbsp; Built by
    <a href="https://hc-portfolio-zeta.vercel.app/" target="_blank">Dr. Piyush Sharma (PT, MHA)</a>
    &nbsp;·&nbsp;
    ...
    <div class="footer-quote">"I am not a developer who learned healthcare. I am a clinician who learned to build."</div>
</div>
""")
